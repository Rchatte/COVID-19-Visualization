...
export default function TreemapPop(props) {
    ...
    */}
    ...
    function setUP(props, svgRef) {
        ...
        let id = "Median_Age_VS_COVID_Deaths"
        let svgName = "Median_Age_VS_COVID_Deaths"
        ...
        var generateColor = d3.scaleLinear()
            .domain([1.14, 27.05])
            .range(colors.parentColor && colors.childrenColor ? [colors.parentColor, colors.childrenColor] : ["#FFFFFF", "#3B3B3B"])
            .clamp(true);
            ...
            d3.csv(temp_url).then(function (data) {
                ...
                d3.csv(freedom_score_csv).then( function(data_input1) {
                    ...
                    filteredScores.map(obj1 => {
                        ...
                        console.log({name : matchingObj["location"], value :matchingObj["total_deaths_per_million"],Old:matchingObj['aged_65_older']})


                        sorted_data.push({name : matchingObj["location"], value :matchingObj["total_deaths_per_million"],Old:matchingObj['aged_65_older']})
                    });
