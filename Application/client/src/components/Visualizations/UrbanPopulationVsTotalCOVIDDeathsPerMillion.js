...
import urb_csv from "./API_SP.URB.TOTL_DS2_en_csv_v2_4645910.csv"
...
export default function TreemapUrbanPopDeath(props) {
    ...
    */}
    ...
    function setUP(props, svgRef) {
        ...
        let id = "UrbanPopulationVsTotalCOVIDDeathsPerMillion"
        let svgName = "UrbanPopulationVsTotalCOVIDDeathsPerMillion"
        ...
        var generateColor = d3.scaleLinear()
            .domain([3, 6443])
            .range(colors.parentColor && colors.childrenColor ? [colors.parentColor, colors.childrenColor] : ["#BAB1A9", "#7A6D63"])
            .clamp(true);
        ...
        function DrawTreemap() {
            ...
            d3.csv(temp_url).then(function (data_) {
                ...
                d3.csv(urb_csv).then( function(data_input1) {
                    ...
                    let blacklist = ["NULL","EUU", "HIC","ZAF", "HIC","LIC","LMC","NAC","UMC","WLD"]



                    const filteredScores = data_input1.filter(obj => !blacklist.includes(obj.Country_Code));

                    filteredScores.map(obj1 => {
                        let matchingObj = data.find(obj2 => obj1["Country_Code"] === obj2["iso_code"]);
                        if (matchingObj){
                            console.log({name : matchingObj["location"], value : matchingObj["total_deaths_per_million"],UrbanPop:obj1['2021']})
                            sorted_data.push({name : matchingObj["location"], value : matchingObj["total_deaths_per_million"],UrbanPop:obj1['2021']})
                ...
                var root = d3.hierarchy(sorted_data).sort(function (a, b) {
                    return b.data.UrbanPop - a.data.UrbanPop
                });
